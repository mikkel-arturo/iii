...
//!
//! The `iii-pubsub` worker registers its config schema under the id
//! `iii-pubsub`, seeds it from the config.yaml block only when no value is
//! stored yet, reads the live value (with `${VAR:default}` expansion) on boot,
...
//! block is seed-only.
//!
//! Tier (see `PubSubWorker::apply_config`): the pub/sub `adapter` is a full
//! hot-swap — a runtime edit rebuilds the backend, re-subscribes every live
//! `subscribe` trigger onto it, and tears down the previous one, without an
//! engine restart.

use anyhow::anyhow;
use serde_json::{Value, json};

use super::{PubSubWorker, config::PubSubModuleConfig};
use crate::{
    engine::{Engine, EngineTrait},
    trigger::Trigger,
};

pub const CONFIG_ID: &str = "iii-pubsub";
pub const CONFIG_FN_ID: &str = "iii-pubsub::on-config-change";
pub const CONFIG_TRIGGER_ID: &str = "iii-pubsub::config-watch";
...
/// Register the `iii-pubsub` configuration entry: schema and metadata refresh on
...
/// `PubSubWorker::start_background_tasks`).
pub async fn register_config(
    engine: &Engine,
    seed: Option<&PubSubModuleConfig>,
) -> anyhow::Result<()> {
    let mut payload = json!({
        ...
        "schema": serde_json::to_value(schemars::schema_for!(PubSubModuleConfig))?,
    });
...
/// `tokio::time::timeout(CONFIG_BUS_TIMEOUT, ...)` (see
/// `PubSubWorker::start_background_tasks` and `PubSubWorker::apply_config`).
pub async fn fetch_config(
    engine: &Engine,
    fallback: &PubSubModuleConfig,
) -> anyhow::Result<PubSubModuleConfig> {
    ...
    let config: PubSubModuleConfig = serde_json::from_value(value)
...
/// Handler body for `iii-pubsub::on-config-change`. Delegates to `apply_config`,
...
/// and acting on a caller-supplied payload would let anyone repoint the backend
/// without updating persisted state. Any failure keeps the previous config and
/// adapter.
pub async fn on_config_change(worker: &PubSubWorker) {
    match worker.apply_config().await {
        Ok(()) => tracing::info!("iii-pubsub configuration re-applied after change"),
        ...
        // cannot loop. Other errors (malformed value, failed adapter build) are
        // deterministic; retrying them would just repeat the failure.
        Err(err) if err.downcast_ref::<tokio::time::error::Elapsed>().is_some() => {
            tracing::error!(
                error = %err,
                "iii-pubsub: configuration apply timed out; retrying once in {APPLY_RETRY_DELAY:?}"
            );
            ...
            tokio::spawn(async move {
                ...
                if !worker.is_active() {
                    tracing::debug!(
                        "iii-pubsub: worker no longer active; skipping configuration apply retry"
                    );
                    return;
                }
                match worker.apply_config().await {
                    Ok(()) => tracing::info!("iii-pubsub configuration re-applied on retry"),
                    Err(err) => tracing::error!(
                        error = %err,
                        "iii-pubsub: configuration apply retry failed; keeping previous config"
                    ),
                }
            });
        }
        Err(err) => tracing::error!(
            error = %err,
            "iii-pubsub: failed to apply changed configuration; keeping previous config"
        ),
    }
}

/// Subscribe to `configuration:updated` events for the `iii-pubsub` entry. The
...
mod tests {
    ...
    async fn register_seeds_initial_value_when_nothing_stored() {
        ...
        let seed = PubSubModuleConfig {
            adapter: Some(crate::workers::traits::AdapterEntry {
                name: "redis".to_string(),
                config: Some(json!({ "redis_url": "redis://seed:6379" })),
            }),
        };
    ...
    async fn register_omits_initial_value_when_value_stored() {
        ...
        let engine = Arc::new(Engine::new());
        let mut registered =
            stub_configuration(&engine, Some(json!({ "adapter": { "name": "local" } })));

        register_config(&engine, Some(&PubSubModuleConfig::default()))
            .await
            .unwrap();
    ...
    async fn fetch_config_falls_back_when_not_found() {
        ...
        let fallback = PubSubModuleConfig {
            adapter: Some(crate::workers::traits::AdapterEntry {
                name: "redis".to_string(),
                config: None,
            }),
    ...
    async fn fetch_config_falls_back_on_null_value() {
        ...
        let config = fetch_config(&engine, &PubSubModuleConfig::default())
            .await
            .unwrap();
    ...
    async fn fetch_config_errors_on_malformed_value() {
        ensure_default_meter();
        let engine = Arc::new(Engine::new());
        // Unknown top-level key → deny_unknown_fields rejects it.
        let _registered = stub_configuration(&engine, Some(json!({ "bogus_field": 1 })));

        let result = fetch_config(&engine, &PubSubModuleConfig::default()).await;
    ...
    async fn on_config_change_keeps_previous_config_on_malformed_value() {
        ...
        let worker = PubSubWorker::for_test(
            engine.clone(),
            Some(json!({ "adapter": { "name": "local" } })),
        )
        .await
        .expect("pubsub worker");
        // Mark active so apply_config runs the full path (not the inactive gate).
        worker.mark_active_for_test();
